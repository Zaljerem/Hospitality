...
internal class LordToil_VisitPoint : LordToil
{
    ...
    private void Arrive()
    {
        ...
        foreach (var pawn in lord.ownedPawns)
        {
            ...
            var expectations = tweak + regularity;
    ...
    private void Leave()
    {
        ...
        var hostile = lord.faction.RelationWith(Faction.OfPlayer).kind == FactionRelationKind.Hostile;
    ...
    {
        var targetGoodwill = Mathf.RoundToInt(Mathf.Lerp(-100, 100, score));
        var goodwillChangeMax = Mathf.Lerp(3, 24, Mathf.InverseLerp(1, 8, visitorCount));
        float currentGoodwill = faction.GoodwillWith(Faction.OfPlayer);
        var offset = targetGoodwill - currentGoodwill;
        var goodwillChange = Mathf.RoundToInt(Mathf.Clamp(offset, -goodwillChangeMax, goodwillChangeMax));
    ...
    private static float PlanRevisit(Faction faction, float targetGoodwill, Map currentMap, bool sentAway)
    {
        ...
        var randomVisitMap = Rand.Value < 0.1f ? Find.Maps.Where(m => m.IsPlayerHome).RandomElement() : currentMap;
